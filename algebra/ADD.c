...
switch(test_counter)
{
  ...
  COLLECT_FINDINGS
  MEMORY_AT_ADDRESS(arguments[0]) = 1;
  //----------------------
...
MEMORY_AT_ADDRESS(arguments[2]) = working_variable < MEMORY_AT_ADDRESS(arguments[1]);
...
#define DESCRIPTION \
Adds the second argument to the first argument. The third argument is set to 1 if the instruction overflows, and 0 if it doesnt.
